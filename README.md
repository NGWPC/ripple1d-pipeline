...
### 1. Environment file (`.env`) — required machine/enviornment specific settings
...
To change a behavior value **just for your machine** copy `config.example.yaml` to `config.yaml` at the reppo root and copy the keys you want to override from  `/ripple1d_pipeline/default_config.yaml`. These settings are deep-merged over the defaults.
