...
"""
taking upstream because
doing one by one because the code is simpler and crs can be different
multi point to first point
datum not sure
normalized for better analytics and querying
gpkgs if working with data on cloud have severe performance issues
do hive partitioning and make a service
these are only us rc points for upstream, we can dig into HEC ras results or do raster querying to generate more points
"""
...
def create_rc_points_parquet(ripple_gpkg_path, submodels_dir, output_parquet_path, huc_id, collection_id):
    ...
    for reach_id in reach_ids:
        ...
        # Skip if submodel GPKG doesn't exist
        ...
        try:
            # Execute and insert the spatial query result directly into the temp table
            query = f"""
            WITH xs AS (
            ...
            INSERT INTO points
            SELECT
                {int_reach_id} as reach_id, pr.model, xs.source_river, xs.source_reach,
                xs.source_river_station,
                (SELECT geom FROM (SELECT unnest(st_dump(ST_Intersection(xs.geom, reach.reach_geom)), recursive := true) LIMIT 1))
            FROM xs, reach, process pr
            WHERE ST_Intersects(xs.geom, reach.reach_geom)
            """
            conn.execute(query)
    ...
    conn.execute(
        ...
        CREATE TEMP TABLE us_rcs AS (
            ...
            FROM st_read('{ripple_gpkg_path}', layer='rating_curves')
...
def main():
    ...
    upload_file_to_s3(
        ...
        s3_path=f"{args.s3_output_prefix}/{args.collection_id}/rating_curves.parquet",
    )
