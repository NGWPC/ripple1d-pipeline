job "ripple_test-failed-collections-3" {
  datacenters = ["dc1"]
...
echo "Copy Updated batch_ripple_pipeline.py file" &&^
echo F|xcopy /Y /F {{ env "NOMAD_TASK_DIR" }}\\ripple1d-pipeline\\batch_ripple_pipeline.py C:\\ripple1d-pipeline\\batch_ripple_pipeline.py &&^
cd /d C:\ripple1d-pipeline &&^
C:\venvs\ripple1d_pipeline\Scripts\activate &&^
echo " Activated ripple1d_pipeline venv" &&^
echo "Submitting python batch_ripple_pipeline.py -l {{ env "NOMAD_META_collection" }} ..." &&^
